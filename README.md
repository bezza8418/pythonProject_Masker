...
### 17. Поиск транзакций по описанию (`filter_by_description`)

Функция для поиска транзакций по ключевым словам в описании с использованием регулярных выражений.
...
found = filter_by_description(transactions, "перевод")
print(f"Найдено транзакций: {len(found)}")
```

### 18. Подсчет транзакций по категориям (`count_by_categories`)

Функция для подсчета количества транзакций в каждой категории.
...
print(result)  # {'Перевод': 2, 'Оплата': 1, 'Покупка': 0}
```

### 19. Интерактивный режим работы (`main`)

Запустите программу для интерактивной работы с транзакциями:
...
- Отфильтровать по статусу (EXECUTED, CANCELED, PENDING)
- Отсортировать по дате
- Отфильтровать только рублевые транзакции
- Выполнить поиск по описанию
- Вывести отформатированный список транзакций
...
Если выборка пуста, программа выведет:
...
│   ├── search.py           # Поиск и категоризации банковских транзакций
...
│   ├── test_search.py      # Тесты для search.py
